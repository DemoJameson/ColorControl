    ...
    partial class AmdPanel : UserControl, IModulePanel
    {
        ...
        private void mnuAmdPresets_Opening(object sender, CancelEventArgs e)
        {
            ...
            if (preset != null)
            {
                ...
                FormUtils.BuildMenuItemAndSub(mnuAmdHDR, "SDR brightness",
                    (preset.SDRBrightness.HasValue ? $"{preset.SDRBrightness.Value}%" : "Unset") + " (click to change)", amdPresetBrightnessMenuItem_Click);
        ...
        private void amdPresetBrightnessMenuItem_Click(object sender, EventArgs e)
        {
            var preset = GetSelectedAmdPreset();
            ...
            if (preset.IsDisplayPreset)
            {
                ...
                _amdService.SetSDRBrightness(preset.displayName, value.Value);
