...
namespace ColorControl.Services.NVIDIA
{
    partial class NvPanel : UserControl, IModulePanel
    {
        ...
        private readonly ColorProfileService _colorProfileService;

        public NvPanel(NvService nvService, NotifyIconManager notifyIconManager, GlobalContext globalContext, RpcClientService rpcService, WinApiAdminService winApiAdminService, KeyboardShortcutDispatcher keyboardShortcutDispatcher, ColorProfileService colorProfileService)
        {
        ...
        private void nvPresetColorProfileMenuItem_Click(object sender, EventArgs e)
        {
            ...
            var profiles = _colorProfileService.GetAllDisplayColorProfiles(display);
            ...
            var none = new ColorProfileDto { Name = "None" };

            profiles.Insert(0, none);

            var value = MessageForms.SelectObjectValue(profiles.FirstOrDefault(p => p.Name == preset.ColorProfileSettings.ProfileName)?.ToString() ?? "None", "Color Profile", profiles);
            ...
            if (preset.IsDisplayPreset)
            {
                _nvService.SetColorProfile(preset.Display, value?.Name);

                UpdateDisplayInfoItems();
                return;
            }

            preset.ColorProfileSettings.ProfileName = value?.Name;
